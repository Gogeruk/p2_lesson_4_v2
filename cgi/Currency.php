...
/**
 * looked up some stuff ftom:
 * https://github.com/fortis/iso-currency/blob/master/src/Currency.php
...
class Currency
{
    ...
    {
        $this -> _setIsoCode($isoCode);
    }

    /**
     * Для свойства isoCode написать гэттер и сеттер.
     */
    private function _setIsoCode($isoCode)
    {
        /**
         * В случае неверного формата названия валюты
         * выбросить InvalidArgumentException.
         */
        if (preg_match('/^[a-zA-Z0-9]+$/', $isoCode) !== 1) {
            throw new InvalidArgumentException(self::ERRORS[0]);
        }
        if (false === array_key_exists($isoCode, self::CURRENCIES)) {
            throw new InvalidArgumentException(self::ERRORS[1]);
        }
     ...
     */
    public function equals(Currency $curr_1)
    {
        if ($this-> _isoCode  == $curr_1-> _isoCode) {
            return true;
        }else {
            return false;
        }
    ...
    const ERRORS = [
        "The currency code must be alphanumeric and non empty.",
